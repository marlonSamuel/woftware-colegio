              ...
              <v-container grid-list-md>
                <v-layout wrap>
                  <v-flex xs12 sm12 md12>
                    <v-text-field v-model="form.seccion" 
                        label="Seccion"
...
export default {
  ...
  methods: {
    ...
    destroy(data){
      let self = this
      self.$confirm('Seguro que desea eliminar sección '+ data.seccion+'?').then(res => {
        self.loading = true
            self.$store.state.services.seccionService
            .destroy(data)
            .then(r => {
    ...
    mapData(data){
        ...
        self.form.seccion = data.seccion
    },
  ...
  computed: {
    setTitle(){
      let self = this
      return self.form.id !== null ? self.form.seccion : 'Nuevo Registro'
    }
  },
};
</script>