  ...
  <v-layout align-start v-loading="loading">
    <v-flex>
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title>Consulta Pagos </v-toolbar-title>
          <v-divider
          class="mx-2"
          inset
          vertical
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Nombres apoderado', value: 'apoderado_name' },
        { text: 'Nombres alumno', value: 'alumno_name', width: "400px" },
  ...
  methods: {
    ...
    getByFechas() {
      ...
      self.loading = true
      self.$store.state.services.pagoService
        .getByFechas(self.inicio, self.fin)
        .then(r => {
          self.loading = false
          self.items = r.data
          self.items.forEach(function(item){
    ...
    buscar () {
      ...
      self.inicio == '' && self.fin == '' ? self.getAll() : self.getByFechas()
    },
